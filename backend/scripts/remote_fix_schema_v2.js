...
console.log('⚡ Applying Schema Fix (V2 - Safe Syntax)...');

conn.on('ready', () => {
    console.log('✅ SSH Connection established.');
    
    // We will write a temp JS file remotely instead of inline execution to avoid escaping hell
    const scriptContent = `
        ...
        (async () => {
            ...
            } catch (e) { console.log('ℹ️ bikes.added_at might exist'); }
    ...
    // Write file remotely using cat with heredoc, but carefully escaped
    // Actually, safest is to use sftp or simple echo line by line, or base64 decode
    // Let's use fs.writeFile locally then upload.
    
    const fs = require('fs');
    const path = require('path');
    const localPath = path.join(__dirname, 'temp_fix_schema.js');
    fs.writeFileSync(localPath, scriptContent);
    
    conn.sftp((err, sftp) => {
        if (err) throw err;
        const remotePath = '/root/eubike/backend/scripts/temp_fix_schema.js';
        ...
        sftp.fastPut(localPath, remotePath, (err) => {
            ...
            // Execute
            conn.exec(`cd /root/eubike/backend && node scripts/temp_fix_schema.js`, (err, stream) => {
                if (err) throw err;
                stream.on('close', (code) => {
                    console.log('🎁 Fix Script Finished with code ' + code);
                    fs.unlinkSync(localPath); // Clean up local
