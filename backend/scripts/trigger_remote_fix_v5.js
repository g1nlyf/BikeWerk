...
console.log('🔌 Connecting to remote server to Upload & Apply Schema V5...');
...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        const filesToUpload = [
            {
                local: path.join(__dirname, 'remote_fix_schema_v5.js'),
                remote: '/root/eubike/backend/scripts/remote_fix_schema_v5.js'
            },
            {
                local: path.join(__dirname, '../src/js/mysql-config.js'),
                remote: '/root/eubike/backend/src/js/mysql-config.js'
            }
        ];
    ...
    function executeFixScript() {
        const cmd = 'cd /root/eubike/backend && node scripts/remote_fix_schema_v5.js';
