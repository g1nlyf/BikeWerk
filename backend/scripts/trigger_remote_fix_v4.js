...
console.log('🔌 Connecting to remote server to Upload & Apply Schema V4...');
...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        const filesToUpload = [
            {
                local: path.join(__dirname, 'remote_fix_schema_v4.js'),
                remote: '/root/eubike/backend/scripts/remote_fix_schema_v4.js'
            },
            {
    ...
    function executeFixScript() {
        const cmd = 'cd /root/eubike/backend && node scripts/remote_fix_schema_v4.js';
