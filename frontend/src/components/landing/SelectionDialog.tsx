...
function loadState(): WizardState {
  ...
  return { name: "", method: null, value: "", notes: "" };
}
...
export default function SelectionDialog() {
  ...
  React.useEffect(() => {
    if (state.selectionOpen) {
      try { metricsApi.sendEvents([{ type: "selection_overlay_shown" }]); } catch { void 0 }
    }
  }, [state.selectionOpen]);
  ...
  return (
    <Dialog open={state.selectionOpen} onOpenChange={(o) => { if (!o) closeAll(); }}>
      <DialogContent className="sm:max-w-xl md:max-w-2xl rounded-3xl p-6 md:p-7">
        <DialogHeader>
          <DialogTitle className="text-2xl md:text-3xl font-semibold">Подберем велосипед бесплатно!</DialogTitle>
          <DialogDescription className="sr-only">Заявка на подбор велосипеда</DialogDescription>
        </DialogHeader>
        <div className="mt-3">
          <Card className="rounded-3xl border-0 shadow-none">
            <CardHeader className="px-0 pt-0">
              <CardTitle className="text-xl md:text-2xl">Ваши контакты</CardTitle>
            </CardHeader>
            <CardContent className="px-0 space-y-6">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Share2 className="h-4 w-4" /> Мы свяжемся с вами в ближайшее время и предложим варианты.
              </div>
              <Separator />
              
              <div className="space-y-2">
                <Label htmlFor="name">Как к вам обращаться?</Label>
                <Input id="name" placeholder="Иван" value={wiz.name}
              ...
              <div className="space-y-3">
                <div className="text-sm font-medium">Куда вам удобнее написать?</div>
                <div className="grid grid-cols-2 gap-2">
                  {(["telegram","whatsapp","email","call"] as ContactMethod[]).map(m => (
                    <button key={m} type="button"
                      onClick={() => setWiz(s => ({ ...s, method: m }))}
                      className={"rounded-xl border px-3 py-3 text-sm hover:bg-muted/60 transition-colors " + (wiz.method===m?"border-primary bg-primary/5 font-medium":"border-muted")}
                    >{m === "telegram" ? "Telegram" : m === "whatsapp" ? "WhatsApp" : m === "email" ? "Email" : "Позвонить мне"}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                 <Label htmlFor="notes">Пожелания (необязательно)</Label>
                 <Textarea 
                   id="notes" 
                   placeholder="Например: хочу гравийник до 1500 евро, рост 180 см..." 
                   value={wiz.notes}
                   onChange={e => setWiz(s => ({ ...s, notes: e.target.value }))}
              ...
              {error && (
                <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
                  ...
                  <pre className="mt-2 overflow-auto max-h-60 text-xs bg-white/50 p-2 rounded-md">{JSON.stringify(error.raw, null, 2)}</pre>
                </div>
              )}

              <div className="pt-2">
                <Button disabled={!nameOk || !valueOk || !wiz.method || submitting}
                  className="w-full rounded-xl py-6 text-lg font-bold shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all"
                  onClick={submit}
                >
                  {submitting ? "Отправляем..." : <>Оставить заявку <ArrowRight className="h-5 w-5 ml-2" /></>}
                </Button>
              </div>
              <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground text-center">
                <CheckCircle className="h-3.5 w-3.5" /> Бесплатно. Без обязательств.
              </div>
            </CardContent>
          </Card>
        </div>
